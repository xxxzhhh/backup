...
public:
    ...
    // packer: 文件打包器
    // rootDirectory: 根目录
    FileInformation(const std::string &relativePath, FilePacker *packer, const std::string &rootDirectory);
    // 打开一个文件用于还原
    // packer: 文件打包器
    // rootDirectory: 根目录
    FileInformation(FilePacker *packer, const std::string &rootDirectory);
    ~FileInformation();
    // 备份文件
    StatusCode Backup();
    // 还原文件
